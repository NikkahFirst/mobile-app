...
serve(async (req) => {
  ...
  try {
    ...
    if (payoutsError) {
      ...
      return new Response(
        JSON.stringify({ error: "Failed to fetch affiliate payouts" }),
        { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    console.error("Error fetching affiliate payout data:", error);
    return new Response(
      JSON.stringify({ error: error.message || "Failed to fetch affiliate payouts" }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  }
});
